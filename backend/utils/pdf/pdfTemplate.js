...
export default function ({
  ...
  margin: 20px 0 15px 50%;
    ...
    <p style="font-size: 12px">${profile?.address ? profile.address : ""}</p> 
    <p style="font-size: 12px">${profile?.city ? profile.city : ""}</p>
    <p style="font-size: 12px">${profile?.country ? profile.country : ""}</p>
</div>

<div className="column">
<h3 style="font-size: 14px; text-transform: uppercase;"> <strong>Billing To: </strong></h3>
...
${document?.billingItems?.map(
  (item, index) =>
    ...
    <tr style='background-color:black; color:white'>
    <th style='font-size:12px; tex-align:center'>Cost Summary</th>
  ...
  <tr>
    <td style="font-size: 10px">VAT/Sales Tax (${document?.rates}%):</td>
    <td style="text-align: right; font-size: 10px; font-weight: 700">${
      document?.currency
    }
